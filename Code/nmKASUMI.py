...
class Kasumi:
    ...
    def enc(self, plaintext, start, end):
    ...
    def dec(self, ciphertext, start, end):
...
def test():
    ...
    encrypted = my_kasumi.enc(text, 1, 8)
    ...
    decrypted = my_kasumi.dec(encrypted, 7, 0)
    print('decrypted', hex(decrypted))

def sandwichEnc(text, key):
    myKasumi = Kasumi()
    myKasumi.set_key(key)

    x = myKasumi.enc(text, 1, 3)
    y = myKasumi.enc(text, 1, 4)
    c = myKasumi.enc(text, 1, 7)
...
def sandwichDenc(text, key):
    ...
    y = myKasumi.dec(text, 7, 4)
    x = myKasumi.dec(text, 7, 3)
    p = myKasumi.dec(text, 7, 0)
